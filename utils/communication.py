...
def _all_to_all(
    ...
    # rank0 input_list = [A0,A1];rank1 input_list = [B0,B1]
    # rank0 output_list = [A0,B1];rank1 output_list = [A1,B0]
...
def prepare_sequence_parallel_data_wanx(
    ...
    def prepare(hidden_states, encoder_hidden_states, uncond_text_states, image_embeds, latents_condition):
        ...
        # print(rank,'------:',hidden_states.shape,encoder_hidden_states.shape,uncond_text_states.shape,image_embeds.shape,latents_condition.shape)
    ...
    #     print('................repeat the input data start:................')
    #     print("latents.shape:", hidden_states.shape)
    #     print("encoder_hidden_states",encoder_hidden_states.shape)
    #     print("uncond_text_states",uncond_text_states.shape)
    #     print("image_embeds",image_embeds.shape)
    #     print("latents_condition",latents_condition.shape)
    #     print('................repeat the input data end:................')
...
def sp_parallel_dataloader_wrapper_wanx(
    ...
    while True:
        # sp_rank = dist.get_rank() // sp_size  # 计算组内rank
        # sp_group_ranks = list(range(sp_rank * sp_size, (sp_rank + 1) * sp_size))
        # sp_group = dist.new_group(ranks=sp_group_ranks)
        # dist.barrier()
        #--------add-----------#
        for data_item in dataloader:
            # batch = [t.to(device) for t in data_item]
            latents, text_states, uncond_text_states, image_embeds, latents_condition = data_item
            # print('---rank init:',device, dist.get_rank(), latents.shape, text_states.shape, uncond_text_states.shape, image_embeds.shape, latents_condition.shape)
            # print('---rank init:',device, dist.get_rank(), latents.sum(), text_states.sum(), uncond_text_states.sum(), image_embeds.sum(), latents_condition.sum())
            # try:
            #     rank=dist.get_rank()
            #     frame_size_persp = latents.shape[2]//sp_size
            #     latents = latents[:,:,frame_size_persp*(rank%sp_size):frame_size_persp*(rank%sp_size+1),...]
            #     latents_condition = latents_condition[:,:,frame_size_persp*(rank%sp_size):frame_size_persp*(rank%sp_size+1),...]
            # except Exception as e:
            #     print(f"Broadcast failed: rank={dist.get_rank()}, error={str(e)}")
            #     yield (1,2,2,2,2)
            # print('---rank----xxxx~~~',device, dist.get_rank())
            yield latents.to(device), text_states.to(device), uncond_text_states.to(device), image_embeds.to(device), latents_condition.to(device)
            # if dist.get_rank() % sp_size == 0:
            # #     # latents, text_states, uncond_text_states, image_embeds, latents_condition = data_item
            #     batch = [t.to(device) for t in data_item]
            # else:
            #     batch = [torch.zeros_like(t).to(device) for t in data_item]  # 占位张量
            # try:
            #     latents, text_states, uncond_text_states, image_embeds, latents_condition = data_item
            #     print('---rank----xxxx~~~',device, dist.get_rank(), latents.shape, text_states.shape, uncond_text_states.shape, image_embeds.shape, latents_condition.shape)
            #     print('---rank----xxxx~~~',device, dist.get_rank(), latents.sum(), text_states.sum(), uncond_text_states.sum(), image_embeds.sum(), latents_condition.sum())
            # except Exception as e:
            #     print(f"Broadcast failed: rank={dist.get_rank()}, error={str(e)}")
            #     return (1,2,2,2,2)
            # for tensor in batch:
            #     try:
            #         dist.broadcast(tensor, src=sp_rank * sp_size, group=sp_group)
            #     except Exception as e:
            #         print(f"Broadcast failed: rank={dist.get_rank()}, error={str(e)}")
            #         raise
            #     latents, text_states, uncond_text_states, image_embeds, latents_condition = batch
            #     print('---rank----11xxxx~~~',device, dist.get_rank(), latents.sum(), text_states.sum(), uncond_text_states.sum(), image_embeds.sum(), latents_condition.sum())

            # import os
            # rank = int(os.environ["LOCAL_RANK"])
            # # import time
            # # time.sleep(5)
            # print('---rank----xxxx~~~',rank, dist.get_rank(), latents.shape, text_states.shape, uncond_text_states.shape, image_embeds.shape, latents_condition.shape)

            # #--------add------------#
            # latents, text_states, uncond_text_states, image_embeds, latents_condition = data_item
            # print('---cur_devide:',device)
            ...
            # import os
            # rank = int(os.environ["LOCAL_RANK"])
            # print('---rank----xxxx~~~',rank, latents.shape, text_states.shape, uncond_text_states.shape, image_embeds.shape, latents_condition.shape)
            ...
            #         #     print('!!!!!!!iter: ',iter,'st_idx: ',st_idx,' ed_idx: ',ed_idx)
            #         yield (
...
def prepare_sequence_parallel_data_ltx(
    ...
    yield hidden_states, encoder_hidden_states, text_mask, uncond_text_states, uncond_text_mask
