...
class Qwen2Encoder_v2(Qwen2PreTrainedModel):
    supports_gradient_checkpointing = True

    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.vision_token = nn.Parameter(torch.randn(1, config.hidden_size), requires_grad=True)
        self.text_token = nn.Parameter(torch.randn(1, config.hidden_size), requires_grad=True)
    ...
    def forward(self, hidden_states):
        ...
        # Prepend two additional tokens
        vision_token = self.vision_token.unsqueeze(0).repeat(hidden_states.size(0), 1, 1)
        text_token = self.text_token.unsqueeze(0).repeat(hidden_states.size(0), 1, 1)
        hidden_states = torch.cat([vision_token, text_token, hidden_states], dim=1)
